...
%           Creates a cla`ssification tree from the loaded data. Currently
...
% ---  #1 --- %
function varargout = LearningGUIShapelet_v3(varargin)
    ...
    % Last Modified by GUIDE v2.5 22-Jul-2015 14:37:14
...
function LearningGUI_OpeningFcn(hObject, ~, handles, varargin)
    ...
    %Set default window width to 5 secs (choice 3)
    set(handles.WindowWidthPopUp,'value',3);
...
function [shapeletMatrix, featureMatrix] = CreateFeatureDataMatrix(data, ...
    ...
    display(getappdata(0, 'samplingRate'));
    display(windowWidth);
    display(sampleSize);
...
function TestDataStreamButton_Callback(hObject, ~, handles) %#ok<DEFNU>
    ...
    [shapeletMatrix, featureMatrix] = CreateFeatureDataMatrixNoDiff(testData, ...
...
function DisplayConfusionMatrix(handles)
    ...
    if (~isempty(handles.tree) && ~isempty(handles.expectedMatrix) && ...
        ...
        figure;
        %plotconfusion(handles.expectedMatrix, handles.predictedMatrix,handles.activitiesUsed);
        plotconfusion(handles.expectedMatrix, handles.predictedMatrix,handles.activitiesUsed);
    end
end
...
function predictionMatrix = PredictionArrayToMatrix(predictionArray, activityArray,...
    ...
    %change here
    predictionMatrix = zeros(length(activityArray)+1, length(predictionArray));
    ...
    for sample = 1:length(predictionArray)
        ...
        %display(char(predictionArray{sample}));
        %display(strcmp(predictionArray{sample},'Unknown'));
        if (strcmp(predictionArray{sample},'Unknown') == 1)
            activityIndex = length(activityArray)+1;
        else
                ...
                if (~isempty(database{subject}{trial}))
                    ...
                    for activity = 1:length(database{subject}{trial})
                        ...
                        if (~isempty(activityData) && validActivities(activity) == 1)
                            [shapeletMatrix, featureMatrix] = CreateFeatureDataMatrixNoDiff(activityData, ...
...
function PlotPrediction(handles, shapeletMatrix, featureMatrix, classificationVector)
    ...
    %add
    expectedMatrix = PredictionArrayToMatrix(classificationVector,...
            handles.activitiesUsed, 'expected data array');
    predictedMatrix = PredictionArrayToMatrix(predictedActivities, ...
            handles.activitiesUsed, 'predicted data array');
    plotconfusionNew(expectedMatrix, predictedMatrix,handles.activitiesUsed);
...
function plotconfusionNew(a,b,activities)
   % activities
   % a(9,:)
    %disp('Expected Matrix follows:')
  %  a;
    %aSize=size(a);
   % disp('Predicted Matrix follows:')
   % b;  
    %bSize=size(b);
    
    %Prediction is unknown when there is an entry in Row 9 of the Predicted Matrix
   % posn=find(b(9,:)==1);
    %Locate Expected activity for unkowns 
    [m,n] = size(b);
    A = zeros(m+1,m-1);
    [c,d] = size(a);
    if (d == 0)
    ...
    rowtext = {};
    for z = 1:l
         coltext(1,z) = {activities{z}};
        rowtext(z,1) = {activities{z}};
    end
    
    rowtext(l+1,1)={'Unknown'};  
    rowtext(l+2,1)={'Correct%'};
    
    output_matrix=[{'  '} coltext; rowtext data_cells];
...
function [shapeletMatrix, featureMatrix] = CreateFeatureDataMatrixNoDiff(data, ...
    windowWidth, featureArray, validFeatures, useWaitbar)
    ...
    shapeletMatrix = zeros(numOfWindows, sampleSize * numChannels);
    featureMatrix = zeros(numOfWindows, numOfFeatures * numChannels);
        ...
        for channel = 1:numChannels
            ...
            for feature = 1:length(featureArray)
                if (validFeatures(feature) == 1)
                    featureMatrix(windowNumber, numOfFeatures * (channel - 1)...
                        + featureCount) = eval([char(featureArray(feature)),'(dataVector)']);
                    %disp(eval([char(featureArray(feature)),'(dataVector)']));
                    featureCount = featureCount + 1;
                end
            end
