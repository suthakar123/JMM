...
public class PatientResponseSection extends CommonSection {
	...
	public PatientResponseSection(String[] answerArray,
			boolean[] withinRange, byte[] kneeImageBufferArray,String tagDateTime) throws Exception {
		this.answerArray = answerArray;
		this.imageBuffer = kneeImageBufferArray;
	...
	private PdfPCell getResponseCell() {
		...
		appQuestions.setSpacingAfter(10);
	...
	private PdfPCell getImageCell() throws BadElementException,
		...
		Image img = Image.getInstance(imageBuffer);
		//Image img = Image.getInstance(imageBuffer);
		...
		img.scaleToFit(100, 130);
		//img.setScaleToFitHeight(true);
		System.out.println("Image width" + img.getScaledHeight());
		System.out.println("Image height" + img.getScaledWidth());
		Chunk imageFooter = new Chunk(IMAGE_FOOTER, greenSubImageFont);
		Chunk tagDate = new Chunk(tagDateTimeForImage, imageTextFont);
